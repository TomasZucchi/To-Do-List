...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const saveData = () => {
    const taskDescription = document.getElementById("todo").value;
    ...
    // Verifica si se han ingresado el nombre y el email
    ...
    // Verifica si el email tiene un formato correcto
    ...
    // Enviar los datos al servidor para guardarlos en MySQL
    fetch("/addTask", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
      body: JSON.stringify({ taskDescription, dueDate, userName, userEmail }),
    })
  ...
  document.querySelector(".bx-plus").addEventListener("click", saveData);
  ...
  buttonsDiv.addEventListener("click", () => {
    swal({
      ...
      text: "¡Una vez eliminadas, no podrás recuperarlas!",
      icon: "warning",
      buttons: true,
      dangerMode: true,
    }).then((willDelete) => {
      ...
      content: {
        ...
        attributes: {
          ...
          <div class="form__group field">
            ...
            <label for="swal-input-name" class="form__label">Nombre</label>
          </div>
          <div class="form__group field">
